...
object ProcessorTestUtils {
  ...
  def prepareState(
    ...
    lui.foreach(instr => state = executeInstruction(core, instr, state))
    val addi = registers.map {
      case (k, v) => {
    ...
    addi.foreach(instr => state = executeInstruction(core, instr, state))
    return state
  }
  ...
  def executeInstruction(
    ...
    while (!core.io_rvfi.rvfi_valid.peek().litToBoolean) {
      ...
      if (core.io_data.data_req.peek().litToBoolean) {
        ...
        if (!core.io_data.data_we.peek().litToBoolean) {
          ...
          write_data = core.io_data.data_wdata.peek().litValue & mask
          previous_state.data_mem =
            store(previous_state, write_address, mask, write_data)
        }
    ...
    core.io_rvfi.rvfi_rs1_rdata.expect(
      previous_state
        .registers(core.io_rvfi.rvfi_rs1_addr.peek().litValue.toInt)
        .U
    )
    core.io_rvfi.rvfi_rs2_rdata.expect(
      previous_state
        .registers(core.io_rvfi.rvfi_rs2_addr.peek().litValue.toInt)
        .U
    )
    previous_state.registers += (core.io_rvfi.rvfi_rd_addr
      .peek()
      .litValue -> core.io_rvfi.rvfi_rd_wdata.peek().litValue)
